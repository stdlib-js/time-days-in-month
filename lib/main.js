...
function daysInMonth( month, year ) {
	...
	if ( arguments.length === 0 ) {
		...
		if ( isDateObject( month ) ) {
			...
			throw new TypeError( format( '1QBAZ', month ) );
		}
	} else {
		if ( !isString( month ) && !isInteger( month ) ) {
			throw new TypeError( format( '1QBAX', month ) );
		}
		if ( !isInteger( year ) ) {
			throw new TypeError( format( '1QB7f', year ) );
		}
		mon = month;
		yr = year;
	}
	if ( isInteger( mon ) && (mon < 1 || mon > 12) ) {
		throw new RangeError( format( '1QBAa', mon ) );
	}
	mon = lowercase( mon.toString() );
	days = DAYS_IN_MONTH[ mon ];
	if ( days === void 0 ) {
		throw new Error( format( '1QBAb', mon ) );
	}
